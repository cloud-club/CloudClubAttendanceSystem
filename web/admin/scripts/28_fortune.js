...
function renderFortuneValidationSummary(validation) {
  ...
  const uniqueCount = validation.uniqueCount;
  ...
  if (errorCount > 0) {
    const first = validation.errors.slice(0, 8).map(item => `<li>${escapeHtml(item.message)}</li>`).join('');
  ...
  (rows || []).forEach((row, idx) => {
    ...
    if (Object.prototype.hasOwnProperty.call(seen, text)) {
      errors.push({
        ...
        message: `${sourceRowNo}행: ${seen[text]}행과 중복 문구입니다.`
      });
      return;
    }
...
function refreshFortuneExecuteButtonState() {
  ...
  const hasErrors = !!(fortuneValidationState && Array.isArray(fortuneValidationState.errors) && fortuneValidationState.errors.length > 0);
...
function analyzeFortuneInput() {
  ...
  if (validation.errors.length === 0) {
    showBoxMessage('fortuneAnalyzeResult', `✅ 검증 완료: ${validation.validRows.length}건 저장 가능`, true);
  } else {
    showBoxMessage('fortuneAnalyzeResult', `❌ 검증 실패: 오류 ${validation.errors.length}건`, false);
  }
}
...
async function executeFortuneUpload() {
  ...
  if (fortuneValidationState.errors.length > 0) {
  ...
  const baseVersionId = fortuneCurrentVersion ? getFortuneVersionToken(fortuneCurrentVersion) : 'builtin';
  ...
  try {
    ...
    fortunePendingUploadId = '';
    showBoxMessage(resultNodeId, `✅ 운세 저장 완료: ${finalized.rowCount}건 (버전 ${escapeHtml(finalized.newVersionId || '-')})`, true);
    if (confirmed) confirmed.checked = false;
...
async function refreshFortuneManagement() {
  ...
  try {
    ...
    if (handleUnauthorizedError(error)) return;
    showBoxMessage('fortuneSaveResult', `❌ ${escapeHtml(getDisplayErrorMessage(error, '운세 데이터를 불러오지 못했습니다.'))}`, false);
  } finally {
    refreshFortuneExecuteButtonState();
  }
}
