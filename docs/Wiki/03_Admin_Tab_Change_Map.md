...
추가로, 회차 일정이 미리 여러 개 등록되어 있어도 기본 그래프는 **현재 활성화된 출석 회차만** 자동으로 보여주는 것이 맞습니다. 이 기본값은 사용자가 날짜/회차 필터를 직접 바꾸기 전까지만 적용되고, 수동 필터를 건 뒤에는 사용자 선택을 우선합니다. 또한 진행 중 회차의 bar slice를 눌러 하단 멤버 목록을 열어둔 상태에서 30초 자동 새로고침이 돌아와도, 같은 slice를 유지한 채 **멤버 목록이 최신 데이터로 갱신**되어야 합니다.
