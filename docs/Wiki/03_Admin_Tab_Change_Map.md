...
하단 영역은 더 이상 이벤트/개인 drilldown 기본 표가 아니라 **상태별 랭킹 또는 빠른 멤버 필터를 보여주는 단일 드릴다운 카드**입니다. `출석 상태 비율` 도넛은 클릭 시 하단을 상태별 랭킹 카드로 전환하며, 여기서 `출석` slice는 `정시 출석(on_time)` 기준으로 해석합니다. `행사별 출석/지각/결석/유고 분포`, `OB/YB 구성 비율`, `출석 횟수 분포`는 클릭 시 하단 빠른 멤버 필터 카드로 연결되고, `출석일 확인` 모달만 `attendanceDashboardDrilldown(member)`를 사용합니다.

`행사별 출석률`, `평균 출석 시간 추이` 차트는 hover-only로 두어 click drilldown을 하지 않습니다. 백엔드 응답의 `meta.defaultMemberKeys`는 현재 프런트 기본 선택에 사용되지 않는 레거시 호환용 필드이고, 실제 기본 상태는 `selectedMemberKeys=[]` 기준으로 해석합니다. `attendanceDashboardSummary.meta.quickFilter`는 하단 멤버 목록과 상태별 랭킹을 즉시 계산하기 위한 additive field이며, 기존 summary 계약을 대체하지 않습니다.
