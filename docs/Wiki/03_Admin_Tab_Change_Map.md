...
## 관리자 공통 레이아웃 계약
관리자 10개 탭은 기능 구현 파일이 서로 달라도 하나의 공통 앱 셸을 사용합니다. 레이아웃이나 탭을 수정할 때는 아래 계약을 탭별 기능 계약과 함께 확인합니다.

1. 상단바와 메뉴
- `web/admin/index.html`의 `#adminTabNav`가 데스크톱과 모바일이 공유하는 유일한 탭 DOM입니다.
- 데스크톱에서는 로고를 왼쪽, 탭 메뉴를 오른쪽에 두며 메뉴가 중앙의 별도 블록을 차지하지 않습니다.
- 모바일에서는 같은 탭 DOM을 오른쪽 드로어로 표시합니다. 모바일용 탭을 복제해서 만들지 않습니다.

2. 탭 접근성과 상태
- 탭 버튼은 `role="tab"`, 패널은 `role="tabpanel"`로 연결합니다.
- 활성 탭의 `aria-selected`, `tabindex`, 패널 `aria-hidden`은 `web/admin/scripts/07_admin_shell.js`에서 동기화합니다.
- Arrow 키, Home/End, Escape, 메뉴 닫기 후 포커스 복귀 동작을 유지합니다.

3. 밀도와 반응형 기준
- 관리자 컨테이너 최대 폭은 1760px이고, 데스크톱 카드 간격은 16px을 기본으로 합니다.
- 1200px 이상부터 공통 유틸리티 영역을 가로로 사용하며, 각 탭의 카드 수와 성격에 따라 1000/1200/1440/1600px에서 다열 그리드를 적용합니다.
- 768px 이하에서는 모든 탭을 단일열로 되돌리고 44px 이상의 조작 영역과 세로 스크롤을 유지합니다.
- 어떤 뷰포트에서도 `document.scrollWidth`가 뷰포트 폭을 넘지 않아야 합니다.

4. 변경 시 필수 회귀 검사
- 탭을 추가하거나 이름을 바꾸면 `scripts/admin_compact_layout_test.js`의 탭 수, ARIA 연결, 스크립트 로딩 순서를 함께 갱신합니다.
- `scripts/run_doublecheck.sh`의 Gate 0-B를 실행해 공통 셸 구조가 유지되는지 확인합니다.
- 375, 768, 1280, 1920px에서 모든 탭의 단일열·다열 전환과 모바일 드로어를 확인합니다.

5. 최근 공통 UI 보정
- 로그인 푸터의 연락처 정본은 `cloudclub2022@cloudclub.kr`이며, 공식 설명·소셜 링크·정책 링크·Contact를 함께 유지합니다.
- `출석현황`의 출석 조회·순위는 데스크톱에서 하나의 보조 행으로 배치하고, 1199px 이하에서만 한 열로 되돌립니다.
- `일정 관리`의 Google Sheets 링크는 초록색 success 버튼과 접근 안내 note를 함께 사용합니다. 시트 직접 수정 주의 문구는 UI 계약이며, 시트 권한 자체를 변경하지 않습니다.
- 1600px 이상에서 일정 목록·캘린더 카드 높이를 동일하게 유지하며, 일정 목록 시작 시각은 `MM-DD HH:mm`으로 축약합니다.

이 공통 셸은 UI 전용 계약입니다. API 액션, 권한 판정, 시트 읽기·쓰기는 계속 `Appsscript/*`가 정본이며, 공통 레이아웃만 바뀐 경우 Apps Script를 다시 배포하지 않습니다.
...
## 속도 최적화 관점의 추가 기준
최근 기준에서 관리자 탭 수정은 기능 맵만이 아니라 “탭이 언제 데이터를 불러와야 하는가”까지 함께 봐야 합니다.

1. `attend` 탭
- 기본 진입은 `scheduleList`와 현재 출석 세션이 우선입니다.
- 수동 승인 상태는 `graduationReport` 전체가 아니라 `manualApproveStatus` 전용 액션으로 읽는 것이 현재 기준입니다.
- 수동 승인 패널은 첫 상호작용 또는 유휴 시점에만 상세 데이터를 채우므로, 초기 화면이 약간 더 빨라지는 것이 정상입니다.

2. `status` 탭
- 기본 회차/기간은 가능하면 프런트가 먼저 확정하고 summary를 요청합니다.
- `summary -> 범위 자동조정 -> summary 재호출`은 최대 1회 보정까지만 허용하는 것이 현재 기준입니다.
- 평균 추이는 여전히 “선택 없으면 전체 평균”이어야 합니다.

3. QR/운세/관리자/변수 탭
- 비가시 탭 데이터는 초기 관리자 진입 때 선로딩하지 않고 실제 탭 진입 시점에 읽는 것이 맞습니다.

상세 성능 기준과 배경은 [07_Admin_Performance_Optimization_Guide.md](./07_Admin_Performance_Optimization_Guide.md)에서 확인합니다.
