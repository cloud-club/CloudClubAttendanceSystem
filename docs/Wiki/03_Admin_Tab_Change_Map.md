...
하단 영역은 더 이상 이벤트/개인 drilldown 기본 표가 아니라 **상태별 랭킹 또는 빠른 멤버 필터를 보여주는 단일 드릴다운 카드**입니다. `출석 상태 비율` 도넛은 기본적으로 하단을 상태별 랭킹 카드로 전환하며, 여기서 `출석` slice는 `정시 출석(on_time)` 기준으로 해석합니다. 이 도넛과 행사별 상태 분포는 `명`보다 **상태 발생 횟수(`회`)** 기준으로 읽는 것이 맞습니다. 단, **진행 중 회차의 미기록 인원은 `pending(미확정)` 회색 slice**로 계산하고, 이 slice는 랭킹이 아니라 하단 멤버 빠른 필터로 연결됩니다. `행사별 출석/지각/결석/유고/미확정 분포`, `OB/YB 구성 비율`, `출석 횟수 분포`도 하단 빠른 멤버 필터 카드로 연결되고, `출석일 확인` 모달만 `attendanceDashboardDrilldown(member)`를 사용합니다.

`행사별 출석률`, `평균 출석 시간 추이` 차트는 계속 hover-only로 두며 click drilldown을 하지 않습니다. 또한 이 둘은 **종료 회차 기준 확정 지표**로 유지되고, 진행 중 회차의 회색 `pending`은 `행사별 상태 분포`와 `출석 상태 비율`에만 반영됩니다. 백엔드 응답의 `meta.defaultMemberKeys`는 현재 프런트 기본 선택에 사용되지 않는 레거시 호환용 필드이고, 실제 기본 상태는 `selectedMemberKeys=[]` 기준으로 해석합니다. `attendanceDashboardSummary.meta.quickFilter`는 하단 멤버 목록과 상태별 랭킹을 즉시 계산하기 위한 additive field이며, 기존 summary 계약을 대체하지 않습니다.
...
10. 출석 상태 비율 donut의 `출석/지각/결석/유고` 클릭이 모두 하단 상태별 랭킹으로 즉시 전환되는지 확인
11. OB/YB 구성 비율 donut 클릭 시 하단 멤버 목록이 즉시 갱신되는지 확인
12. 출석 횟수 분포 donut의 exact bucket과 `기타` 클릭이 모두 올바른 멤버 목록을 보여주는지 확인
13. 하단 드릴다운의 `출석일 확인`만 API 호출을 사용하고, 같은 멤버 재조회 시 캐시가 적용되는지 확인
14. `행사별 출석률`, `평균 출석 시간 추이` 차트는 click drilldown을 시도하지 않는지 확인
