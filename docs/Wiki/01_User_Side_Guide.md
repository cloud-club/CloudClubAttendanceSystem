...
행사 일정은 별도 내부 스크롤 없이 전체 회차가 이어서 표시됩니다. 수료 조건 탭으로 바로 들어와 조항을 열어도 같은 일정 API에서 현재 시즌 정책을 불러옵니다. 팝업은 Escape 또는 배경 클릭으로 닫을 수 있으며 닫은 뒤에는 실행했던 버튼으로 초점이 돌아갑니다.
...
4. 회차별 출석 상세는 접힌 영역 없이 결과에 항상 표시됩니다. 출석·지각·결석의 공개 사유가 있는 행만 누르면 상태·날짜·시간·사유를 상세 다이얼로그에서 확인할 수 있습니다.
5. 유고는 개인정보 보호를 위해 상태만 `유고`로 표시하며 사유와 시트 메모를 학생 API와 화면에 전달하지 않습니다.
6. 상세 다이얼로그는 `Escape`로 닫을 수 있고, 키보드 포커스는 다이얼로그 안에서 순환한 뒤 닫을 때 연결된 행으로 돌아갑니다.
7. 하단 랭킹 보드에서 기존 시즌 상위 순위를 확인합니다.

### 3) 수료 조건 확인
1. `수료 조건 확인` 탭에서 전화번호를 입력합니다. 마지막으로 사용한 번호는 전화번호 입력이 있는 학생 탭의 입력란에 함께 반영됩니다.
...
5. 필수 회차 충족 여부, 환산 결석률, 현재 기준 수료 가능 여부를 확인합니다.
...
### 화면 크기별 배치
- 학생 정보 구조는 항상 `출석하기 → 출석 현황 → 행사 일정 → 수료 조건 확인` 4개 탭 순서를 유지합니다.
