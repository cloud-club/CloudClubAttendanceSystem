...
시즌 회차가 많아도 창 자체는 화면 높이 안에 머물고 표 본문만 세로·가로로 스크롤됩니다. Escape 또는 배경 클릭으로 닫을 수 있으며, 닫은 뒤에는 원래 누른 `출석일 확인` 버튼으로 초점이 돌아갑니다.

## 출석현황 화면 밀도

- 출석 현황 대시보드는 기본 열린 상태이며 제목 행의 꺾쇠로 전체를 접거나 펼칠 수 있습니다.
...
- 유고 사유의 운영 근거를 관리자 전용 메모로 기록합니다. 학생 화면에는 개인정보 보호를 위해 사유 없이 `유고` 상태만 표시됩니다.
