...
이때 모든 차트가 같은 기준으로 바뀌는 것은 아닙니다. `행사별 출석/지각/결석/유고 분포`와 `출석 상태 비율`은 진행 중 회차의 `pending`을 함께 보여주지만, `행사별 출석률`과 `평균 출석 시간 추이`는 계속 **종료 회차 기준 확정 지표**로 유지됩니다. 따라서 운영자는 “실시간으로 누가 아직 안 왔는지”는 회색 분포/도넛에서 보고, “최종적인 출석 품질”은 기존 출석률/평균 추이에서 본다고 이해하면 혼동이 적습니다.

또 하나 달라진 점은 단위 해석입니다. `출석 상태 비율`은 이제 사람 수(`명`)보다 **상태 발생 횟수(`회`)** 기준으로 읽는 것이 맞습니다. 반면 하단 멤버 목록이나 상태별 랭킹에서 실제 사람을 세는 부분은 여전히 `명` 기준입니다. 즉 이번 변경은 지표를 더 많이 만든 것이 아니라, **운영자가 실시간 상태와 최종 지표를 서로 다른 층으로 읽을 수 있게 정리한 것**이라고 보는 편이 정확합니다.
