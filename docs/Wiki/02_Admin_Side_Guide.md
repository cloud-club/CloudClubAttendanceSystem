...
또한 실제 행사 운영에서는 전체 일정을 미리 등록해 두는 경우가 많기 때문에, 이제 기본 그래프는 **오늘 현재 활성화된 출석 회차만** 자동으로 보이게 해석하는 것이 맞습니다. 단 이 동작은 기본값일 뿐이고, 운영자가 날짜/회차 필터를 직접 바꾸면 그 선택을 우선합니다. 그래프를 클릭해 하단 드릴다운을 열어둔 상태에서도 자동 새로고침 때문에 선택이 풀리지 않고, 같은 slice가 **최신 데이터로 계속 갱신**되는 것이 이번 기준입니다.
