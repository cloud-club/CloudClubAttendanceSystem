...
3. 그래프 클릭 시 하단 멤버 빠른 필터는 프런트에서 즉시 계산
...
7. 하단 카드는 더 이상 행사/개인 drilldown 표가 아니라, 그래프 클릭으로 열리는 **멤버 빠른 필터 목록**입니다.
8. 빠른 필터를 지원하는 그래프는 `행사별 출석/지각/결석/유고 분포`, `OB/YB 구성 비율`, `출석 횟수 분포` 3개뿐입니다.
9. `행사별 출석률`, `평균 출석 시간 추이` 차트는 hover-only이며 click drilldown을 하지 않습니다.
...
- 빠른 필터는 항상 **마지막으로 클릭한 slice 1개만 유지**되며, 다시 클릭하면 해제된다는 점을 안내할 것
- 하단 멤버 목록은 서버를 다시 조회하지 않고 summary 응답의 quick filter 인덱스에서 즉시 계산된다는 점을 염두에 둘 것
- 개인 회차 이력을 보고 싶다면 하단 멤버 목록의 `출석일 확인` 버튼을 사용하고, 이 모달은 **현재 대시보드 필터 기준 회차만** 보여준다고 설명할 것
