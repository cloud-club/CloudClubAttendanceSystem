...
8. [ ] 행사별 출석/지각/결석/유고 분포의 stacked bar segment 클릭 시 하단 멤버 빠른 필터가 즉시 갱신됨
9. [ ] OB/YB 구성 비율 donut 클릭 시 하단 멤버 빠른 필터가 즉시 갱신됨
10. [ ] 출석 횟수 분포 donut의 exact bucket과 `기타` 클릭이 모두 올바른 멤버 목록을 보여줌
11. [ ] 하단 멤버 빠른 필터에서 `필터 해제`가 즉시 동작하고, 추가 API 호출 없이 목록이 닫힘
12. [ ] 하단 멤버 목록의 `출석일 확인` 클릭 시 현재 대시보드 필터 기준 회차만 모달로 표시됨
13. [ ] 같은 멤버의 `출석일 확인`을 60초 내 재실행하면 캐시된 응답으로 더 빠르게 열림
14. [ ] `행사별 출석률`, `평균 출석 시간 추이` 차트는 hover-only이며 click drilldown을 시도하지 않음
