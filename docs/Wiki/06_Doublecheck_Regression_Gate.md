...
9. [ ] 출석 상태 비율 donut의 `출석/지각/결석/유고` 클릭 시 하단 상태별 랭킹이 즉시 갱신되고, `미확정` 클릭은 멤버 빠른 필터로 전환됨
...
16. [ ] `행사별 출석률`, `평균 출석 시간 추이` 차트는 hover-only이며 click drilldown을 시도하지 않고, 진행 중 회차의 회색 `미확정`은 여기에 섞이지 않음
17. [ ] 출석현황 탭이 열려 있는 동안 30초 내 자동 새로고침으로 진행 중 회차의 `미확정` 인원이 갱신됨
18. [ ] 진행 중 회차가 `lateDeadline`을 지나면 회색 `미확정`이 결석으로 전환됨
