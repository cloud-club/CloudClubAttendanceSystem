...
9. [ ] 행사 상태 막대 클릭 후 하단 멤버 목록에 `출석 시간` 컬럼이 `이메일`과 `출석일 확인` 사이에 나타남
10. [ ] `출석/지각` 행은 `HH:MM`, `결석/유고/미확정` 행은 `-`로 보임
11. [ ] 출석 상태 비율 donut에는 `미확정`이 보이지 않고, `출석/지각/결석/유고` 클릭 시 하단 상태별 랭킹이 즉시 갱신됨
12. [ ] `출석`은 정시 출석 횟수 내림차순, `지각`은 지각 횟수 내림차순 + 평균 지각 오프셋 desc, `결석/유고`는 횟수 내림차순으로 보임
13. [ ] OB/YB 구성 비율 donut 클릭 시 하단 멤버 빠른 필터가 즉시 갱신됨
14. [ ] 출석 횟수 분포 donut의 exact bucket과 `기타` 클릭이 모두 올바른 멤버 목록을 보여줌
15. [ ] 하단 드릴다운에서 `필터 해제`가 즉시 동작하고, 추가 API 호출 없이 목록/랭킹이 닫힘
16. [ ] 하단 드릴다운의 `출석일 확인` 클릭 시 현재 대시보드 필터 기준 회차만 모달로 표시됨
17. [ ] 같은 멤버의 `출석일 확인`을 60초 내 재실행하면 캐시된 응답으로 더 빠르게 열림
18. [ ] `행사별 출석률` 차트는 hover-only이며 click drilldown을 시도하지 않고, 진행 중 회차가 있을 때 `현재 반영 인원 / 전체 대상` 기준으로 실시간 갱신됨
19. [ ] `평균 출석 시간 추이` 차트는 hover-only이며 click drilldown을 시도하지 않고, 진행 중 회차가 있을 때 `현재까지 기록된 출석자 평균`으로 실시간 갱신됨
20. [ ] 출석현황 탭이 열려 있는 동안 30초 내 자동 새로고침으로 진행 중 회차의 `미확정` bar, 행사별 출석률, 평균 추이, 상태 비율이 함께 갱신됨
