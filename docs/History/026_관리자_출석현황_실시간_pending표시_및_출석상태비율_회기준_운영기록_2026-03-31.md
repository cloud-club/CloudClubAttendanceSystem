...
flowchart LR
  ...
  B --> C["상태 비율은 명이 아니라 회 기준으로 해석"]
  C --> D["진행 중 회차만 실시간 반영, 확정 지표는 종료 회차 유지"]
  D --> E["30초 자동 갱신 + lateDeadline 이후 absent 전환"]
```
...
3. `출석 상태 비율`을 사람 수(`명`)처럼 해석하면, 실제로는 회차별 상태 발생 횟수를 읽고 있는데도 의미가 흐려질 수 있었다.
...
### 3-3. 실시간화 범위는 제한한다
실시간 반영은 모든 차트에 넣지 않았다.

- **반영**: `행사별 출석/지각/결석/유고 분포`, `출석 상태 비율`
- **유지**: `행사별 출석률`, `평균 출석 시간 추이`, KPI

즉 실시간 상태와 확정 지표를 섞지 않고, 운영자가 **“지금 상황”과 “최종 품질”을 따로 읽게 하는 것**이 핵심이었다.

### 3-4. 상태 비율은 `명`보다 `회/번` 기준으로 읽는다
`출석 상태 비율`과 상태 분포는 사람 수가 아니라 **상태 발생 횟수**를 보여주는 쪽이 더 정확했다. 대신 하단 멤버 목록과 상태별 랭킹은 여전히 실제 사람 수(`명`)로 읽도록 구분했다.
...
- `출석 상태 비율` 도넛에도 회색 `pending` slice를 추가했다.
- 최종 상태 4종(`출석/지각/결석/유고`)은 기존처럼 상태별 랭킹으로 연결하고, `pending`은 **하단 멤버 빠른 필터**로 연결했다.
...
- 최종 지표(`출석률`, `평균 출석 시간`)는 계속 안정적으로 유지된다.
...
- `행사별 출석률`, `평균 출석 시간 추이`는 진행 중 회차를 섞지 않는지
