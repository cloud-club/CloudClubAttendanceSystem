...
> 후속 변경 메모: 이 문서는 평균 출석시간 추이 전환과 그래프 클릭형 빠른 필터까지를 다룹니다. 진행 중 회차의 `pending(미확정)` 실시간 표시, 행사별 출석률/평균 추이의 실시간 반영, 상태 비율의 `회` 기준 해석 조정, 30초 자동 갱신은 [026_관리자_출석현황_실시간_pending표시_및_출석상태비율_회기준_운영기록_2026-03-31.md](./026_관리자_출석현황_실시간_pending표시_및_출석상태비율_회기준_운영기록_2026-03-31.md)를 우선 참고하세요.
