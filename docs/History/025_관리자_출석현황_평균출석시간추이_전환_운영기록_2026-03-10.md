...
## 1. 배경
기존 관리자 `출석현황` 탭은 통계를 보는 용도로는 충분했지만, 실제 운영 흐름에서는 한 가지 단절이 남아 있었다.

그래프를 보다 보면 자연스럽게 클릭하게 되는데, 막상 클릭했을 때 **그 시점에 누가 왔고 누가 안 왔는지 바로 보이지 않았다.**
운영진 입장에서는 차트 수치 자체보다, 그 수치에 해당하는 실제 사람을 곧바로 확인할 수 있어야 화면의 가치가 커진다.

다만 여기서 별도의 새 패널이나 추가 UI를 계속 늘리는 것은 적절하지 않았다.

1. 화면이 더 복잡해지고
2. 남에게 보여줄 때 정보의 초점이 흐려지고
3. 단순 출석 확인용으로는 현재 대시보드 하단 영역의 효율이 낮았다

그래서 이번 후속 개선은 새 UI를 하나 더 만드는 대신, **기존 하단 드릴다운 영역을 즉시 멤버 확인용 리스트로 재활용**하는 방향으로 정리했다.

이 작업의 목적은 아래 두 가지였다.

1. 대시보드 영역 하단의 리스트 출력 방식 개선
2. 데이터 활용도 및 접근 속도 향상

결론적으로 이번 통합 변경의 핵심은 다음 한 문장으로 요약된다.

**이제 대시보드는 통계만 보는 영역이 아니라, 그래프에서 이상 징후를 보자마자 사람까지 바로 확인하는 운영 도구다.**
...
3. 멤버 2명 이상 선택: 선택 집합만의 평균값 1개 라인

또한 기본 3명 자동 선택을 제거해, 진입 직후에도 차트가 비지 않고 **전체 평균**을 기본 상태로 보여주도록 바꿨다.

### 2-2. 2차 변경: 그래프 클릭형 빠른 멤버 필터
후속 개선에서는 하단 영역의 역할을 바꿨다.

기존:
- 행사 drilldown 또는 개인 drilldown 표

현재:
- 그래프 클릭 결과를 즉시 보여주는 **멤버 빠른 필터 카드**

최종 UX는 다음과 같다.

1. `행사별 출석/지각/결석/유고 분포`의 stacked bar segment 클릭
   해당 회차 + 해당 상태의 멤버 목록을 하단에 즉시 표시
2. `OB/YB 구성 비율` donut 클릭
   해당 구성 멤버 목록을 하단에 즉시 표시
3. `출석 횟수 분포` donut 클릭
   exact bucket 또는 `기타`에 해당하는 멤버 목록을 하단에 즉시 표시
4. 하단 목록의 `출석일 확인` 클릭
   해당 멤버의 현재 대시보드 필터 기준 회차 이력을 모달로 표시

같은 slice를 다시 누르면 필터는 해제되고, 활성 slice는 항상 1개만 유지한다.

### 2-3. 백엔드/프런트 계약 정리
이번 개선은 새 API를 만든 작업이 아니다.

1. `attendanceDashboardSummary`는 기존 계약을 유지한 채 `meta.quickFilter`만 additive field로 추가했다.
2. 하단 멤버 목록은 이 `meta.quickFilter` 인덱스를 사용해 **프런트에서 즉시 계산**한다.
3. `attendanceDashboardDrilldown(member)` 계약은 그대로 유지한다.
4. `출석일 확인` 모달만 기존 member drilldown API를 호출하고, 프런트 60초 캐시를 둔다.
5. `행사별 출석률`, `평균 출석 시간 추이` 차트는 hover-only이며 click drilldown을 하지 않는다.

즉 이번 구조는 “summary는 빠른 필터용 인덱스”, “member drilldown은 모달용 상세”로 역할을 분리한 설계다.

## 3. 운영상 의미
이번 후속 개선으로 대시보드의 사용 방식이 달라졌다.

1. 운영자는 수치를 본 뒤 다시 다른 검색 UI로 이동하지 않아도 된다.
2. 그래프에서 바로 멤버를 확인할 수 있어, 현장 대응 속도가 빨라진다.
3. 새로운 화면을 늘리지 않고 기존 하단 영역을 재활용해 화면 밀도를 통제했다.
4. 시연이나 공유 시에도 “왜 이 수치가 나왔는가”를 사람 목록까지 바로 연결해 설명할 수 있다.

운영적으로 중요한 포인트는 다음과 같다.

1. 멤버를 비워 둔 상태는 오류가 아니라 `전체 평균` 기본 상태다.
2. 멤버 선택은 여전히 차트 C 전용 부분집합 필터다.
3. 하단 카드는 더 이상 기본 drilldown 표가 아니라, 그래프 클릭 결과를 보여주는 빠른 멤버 목록이다.
4. 개인 회차 이력이 필요할 때만 `출석일 확인` 모달을 연다.

## 4. 바꾸지 않은 것
이번 통합 변경에서도 아래는 바꾸지 않았다.

1. 학생 페이지 동작
2. 기존 전화번호 기반 출석현황 조회 흐름
3. 랭킹 규칙
4. `attendanceDashboardDrilldown(member)` 응답 계약
5. `attendanceDashboardSummary`의 기존 필드 구조
6. 다른 관리자 탭 동작

즉 기존 계약을 깨지 않고, **summary에 빠른 필터 인덱스를 보강하고 하단 렌더 방식만 바꾼 작업**으로 보는 것이 맞다.
...
5. OB/YB donut 클릭 시 하단 멤버 목록이 즉시 갱신되는지 확인
6. 출석 횟수 분포 donut의 exact bucket과 `기타`가 모두 올바른 멤버 목록을 보여주는지 확인
7. `출석일 확인` 모달만 API 호출을 사용하고, 같은 멤버 재호출 시 60초 캐시가 적용되는지 확인
8. `행사별 출석률`, `평균 출석 시간 추이` 차트는 클릭 반응 없이 hover-only인지 확인
9. 학생 페이지 `출석현황/랭킹`이 기존과 동일한지 확인
